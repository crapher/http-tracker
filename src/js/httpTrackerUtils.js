...
function getManifestDetails() {
  if (!customManifestDetails) {
    ...
    if (manifest) {
      ...
      customManifestDetails.title = `${(manifest.action && manifest.action.default_title) ? manifest.action.default_title : (manifest.browser_action ? manifest.browser_action.default_title : "HTTP Tracker")} (version : ${manifest.version})`;
    }
  }

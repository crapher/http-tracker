...
const reqHeadersBlocking = httpTracker.isFF ? ["requestHeaders"] : ["requestHeaders", "extraHeaders"]; // MV3: non-blocking
...
r.onBeforeSendHeaders.addListener(
  function(details) {
    ...
    details.requestIdEnhanced = details.requestId;
    // MV3 note: modifying or cancelling here isn't allowed without enterprise policy.
    // We'll still log details for visibility.
    try { addModifyRequestHeaders(details); } catch (e) {}
    try { eventTracker.logRequestDetails(details); } catch (e) {}
    // Intentionally return nothing (non-blocking)
